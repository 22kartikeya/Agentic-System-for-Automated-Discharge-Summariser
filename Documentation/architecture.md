...
6. **MCP Elicitation ≠ full HITL** — elicitation is only for **non-blocking** gaps.  
7. **Summary runs only after the release gate** allows it.  
8. **RAG** is a top-level Agno subsystem (five agents), not nested under `agents/`.
...
Validation output
        │
        ├─ Blocking field missing ──────────────► HITL Corrections (no auto-summary)
        │
        ├─ Non-blocking field missing ──────────► MCP Elicitation (Streamlit callback)
        │                                            accept  → continue validation
        │                                            decline → escalate HITL
        │                                            cancel  → abort / escalate
        │
        ├─ Critical cross-val fail ─────────────► Block discharge → mandatory HITL
        │
        └─ Risk score (rules.yaml)
                 │
                 ├─ Low  (≤ low_max) + not blocked ─► Auto-approve → Summary
                 ├─ Medium                           ► Standard HITL → Summary if approved
                 └─ High / hard HITL guardrail       ► Mandatory HITL → Summary only after approve
```

**Rule:** Do not treat “any problem” as elicitation.  
Elicitation is **only** for non-blocking gaps. Blocking / Critical / High risk use full HITL.
...
| Prompts | discharge-extraction · ehr-cross-validation · abbreviation-normalization · summary-generation · rag-answer |
